...
## 🗺️ 目次
1. [ワークスペースの準備](#section-1-workspace)
2. [アプリケーションの生成](#section-2-new)
3. [リモートリポジトリへの接続](#section-3-github)
4. [サーバー起動と表示確認](#section-4-boot)
5. [学習ノート](#section-5-※)

---

| 第0章 ◀（なし） | [⬆ 目次](../index.md#chap-1) | [第2章：認証基盤の構築と日本語化 ▶](./02_authentication_and_i18n.md) |
|:---:|:---:|:---:|
...
## <a id="section-1-workspace"></a>1. ワークスペース（作業ディレクトリ）の準備
...
### コマンドの解説
- `mkdir environment`: `environment` という名前の新しいディレクトリ（フォルダ）を作成します。[※1](#appendix-1)
- `cd environment`: 作成したディレクトリの中に、ターミナルの作業場所を移動させます。[※2](#appendix-2)
...
## <a id="section-2-new"></a>2. アプリケーションの生成
...
- `--css tailwind`: モダンなデザインを素早く実現する Tailwind CSS を導入します。[※3](#appendix-3)
- `--javascript importmap`: ビルド不要で JavaScript を動かせる Importmap を設定します。[※4](#appendix-4)

> 💡 **現場の知恵**：もし既にフォルダを作成済みの場所で展開したい場合は [※5](#appendix-5) を参照。
...
```
> 📚 **根拠と詳細**: なぜこの修正が必要なのか、シンボルの意味などは [※6](#appendix-6) で詳細に解体説明しています。

- **環境の反映**:
[Dir: environment/nexus_mfg]
```bash
cd nexus_mfg
bundle install
```
...
### コマンドの解説
- `bundle install`: `Gemfile` の内容を読み込み、必要なライブラリを調達・固定します。[※7](#appendix-7)
...
## <a id="section-3-github"></a>3. リモートリポジトリ（GitHub）への接続
...
/.vscode/
```
> **負の説明**: 設定しない場合のセキュリティリスクについては [※8](#appendix-8) を参照。

### ステップ2：初回コミット
現在の状態を「最初の歴史」として Git に刻みます。

- **コマンド**:
[environment/nexus_mfg]
```bash
git init
git add .
...
### コマンドの解説
- `git init`: この場所で Git による履歴管理を開始します。[※9](#appendix-9)
- `git add .`: すべての変更を「仮置き場」に登録します。[※10](#appendix-10)
- `git commit -m "..."`: 仮置き場の内容を「歴史」として確定させます。[※11](#appendix-11)

> 💡 **脱出策**：間違えて `add` した時やファイルを消しすぎた時は [※12](#appendix-12) を参照。
...
### コマンドの解説
- `git remote add origin`: GitHub の場所を `origin`（源流）という名前で登録します。[※13](#appendix-13)
- `git push -u origin main`: `main` ブランチの歴史を初めて GitHub へ送信します。`-u` は、次回から `git push` だけで済むようにするための設定です。[※14](#appendix-14)

---

## <a id="section-4-boot"></a>4. アプリケーションの稼働確認
...
> ⚠️ **警告：もしサーバーを止め忘れたら？**
> 次回の起動時に「Address already in use（場所が使われています）」と怒られます。その際の「ゾンビプロセス討伐」の手順は [※15](#appendix-15) を参照してください。

> 💡 **もし `./bin/dev` 自体が失敗したら？**
> 「Foreman が見つからない」等の PATH 関連エラーが出た場合の対策は [※16](#appendix-16) を参照してください。
...
<a id="appendix-6"></a>
### **[[※6 Gemfile プラットフォーム名の解体と Mac での解決理由](#ref-6)]**
- **`platforms: %i[...]`**:
...
<a id="appendix-7"></a>
### **[[※7 bundle install（ライブラリの調達と固定）](#ref-7)]**
...
<a id="appendix-8"></a>
### **[[※8 .gitignore を設定しない場合の具体的恐怖](#ref-8)]**
...
<a id="appendix-10"></a>
### **[[※10 git add .（ステージング / 仮置き場）](#ref-10)]**
Git にとっての「シャッターを切る前の準備」です。
- **例え話**: 集合写真で「誰をアルバムに載せるか」を決めて、ひな壇に並める行為に似ています。まだ歴史には残りません。
- **対象の絞り込み**: 特定のファイルだけを選びたい場合は `git add ファイル名` を使い、`git status` で状態を確認する癖をつけましょう。
👉 [公式リファレンス (Resource Hub) ](../resource_hub.md#tech-git)

<a id="appendix-11"></a>
### **[[※11 git commit -m と「良いコミットメッセージ」](#ref-11)]**
...
<a id="appendix-12"></a>
### **[[※12 git restore / switch（歴史による救済）](#ref-12)]**
「手元でミスをしてしまい、以前の状態に戻せなくなった」時の救済措置です。
#### 手順：ファイルを特定のコミット状態まで戻す
```bash
git restore <ファイル名>
```
これにより、まだコミットしていない最新の変更をすべて捨てて、最後に記録した「健全な状態」まで物理的に巻き戻せます。

#### 推奨：ブランチ移動も近代的に
古い資料では `git checkout` というコマンドが多用されていますが、最新の Git では以下の専用コマンドが推奨されています。
- `git switch`: ブランチを切り替える際に使用。
- `git restore`: ファイルの内容を歴史から復元する際に使用。

**Why Logic**: `checkout` という一つの言葉に「移動」と「復元」の 2 つの役割が混ざっていたことが、初心者の誤操作の一因でした。役割を分けることで、「今ブランチを動かしているのか、ファイルを書き換えているのか」を明確に意識できるようになります。
👉 [公式リファレンス (Resource Hub) ](../resource_hub.md#tech-git)

<a id="appendix-13"></a>
### **[[※13 git remote add origin（遠い場所へのあだ名）](#ref-13)]**
GitHub などの外部サーバーを「リモート」と呼び、最初に同期する場所を慣習的に `origin`（源流）と名付けます。
👉 [公式リファレンス (Resource Hub) ](../resource_hub.md#tech-github)

<a id="appendix-14"></a>
### **[[※14 git push -u（上流工程の固定）](#ref-14)]**
- **なぜ -u ？**: "Set Upstream" の略です。初回に一度設定すれば、次からは `git push` だけ打てば GitHub へ送信されるようになります。
👉 [公式リファレンス (Resource Hub) ](../resource_hub.md#tech-git)

<a id="appendix-15"></a>
### **[[※15 Survival Strategy: ゾンビプロセスの討伐（lsof / kill / ps）](#ref-15)]**
...
- **コマンド**: [Branch: main] [Dir: (任意)] `lsof -i :3000`
- **表示例**:
  ```text
...
<a id="appendix-16"></a>
### **[[※16 Survival Strategy: bin/dev 起動失敗の PATH 対策](#ref-16)]**
...
- **解体**: `eval` はシェルに「この文字列をコマンドとして実行せよ」を命じる組み込み関数です。`$(rbenv init -)` は rbenv の初期化スクリプトを出力するサブコマンドで、`-` はシェルの種類を自動判別するフラグです。
