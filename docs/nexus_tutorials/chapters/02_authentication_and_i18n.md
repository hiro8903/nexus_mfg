...
> 💡 **`available_locales` を設定する理由**
> この設定は、アプリが受け付ける言語を明示的に制限する「ホワイトリスト」の役割を果たします。
> - **安全面**: 不正な言語コードの指定によるエラーを防ぎます。
> - **パフォーマンス面**: 不要な多言語ファイルをメモリに読み込まないようにし、リソースを節約します。
...
### 7.4 全テストの実行と「予期せぬ失敗」の検知（Pain Phase）

基本テストが通っても、プロの開発現場（CI環境：GitHub上での自動テスト環境）を突破できるとは限りません。<a id="ref-16"></a>[**※16**](#appendix-16) ここで「全テスト」を実行し、潜んでいる不備をあぶり出します。

> 💡 **もしここでセキュリティエラーが発生したら？**
> 本文の内容とは無関係な Gem（ライブラリ）の脆弱性が原因で CI が落ちることがあります。その場合は、[**トラブルシューティング：Gem セキュリティエラーの解消**](../troubleshooting/security/gem_vulnerability.md) を参照して解決しましょう。

[environment/nexus_mfg (feat/chapter02-auth-i18n)]
```bash
# システムテストを含む、全てのテストを実行
bin/rails test:all
```

> 💡 **なぜ `test` と `test:all` が分かれているのか？（速度 vs 網羅性）**
> - **`bin/rails test` (高速)**: モデルやコントローラーの動作を「ブラウザを使わず」にチェックします。一瞬で終わるため、コードを一行書くたびに実行して「開発のリズム」を保つのに適しています。
> - **`bin/rails test:all` (低速/網羅)**: 実際にブラウザを立ち上げて動かす「システムテスト」が含まれます。ブラウザの起動は非常に重く時間がかかるため、開発中のリズムを壊さないよう Rails では標準で切り分けられています。
>
> したがって、開発中は `test` を頻繁に使い、**プルリク前や CI 対策として `test:all` で最終確認する**のがプロの使い分けです。

もし以下のような `LoadError` が出たら、それは **「GitHub に送っても確実に失敗する」** というサインです。

```text
cannot load such file -- /.../test/system (LoadError)
```

> 💡 **なぜ `bin/rails test` では気づけなかったのか？**
> - **`bin/rails test`**: ユニットテスト（Model等）と統合テスト（Controller等）のみを実行します。高速ですが、ブラウザを使う「システムテスト」はスキップされます。
> - **`bin/rails test:all`**: システムテストを含む全てを実行します。低速ですが、プルリク前の最終確認には不可欠です。

### 7.5 システムディレクトリの保護（.keep）

上のエラーの原因は、Git の **「空のフォルダは無視する」** という性質にあります。`test/system` が空だと GitHub に送信されず、CI 環境で「フォルダがない！」と怒られてしまうのです。

[environment/nexus_mfg (feat/chapter02-auth-i18n)]
```bash
# ディレクトリを強制作成し、おまじない（.keep）を置く
mkdir -p test/system
touch test/system/.keep
git add test/system/.keep
```

### 7.6 Lint（RuboCop）による磨き上げ

最後に、コードの「見た目」を規約通りに整えます。

[environment/nexus_mfg (feat/chapter02-auth-i18n)]
```bash
# プロジェクト内の全てのファイルを対象に、安全な修正（-a）を自動実行する
bundle exec rubocop -a <a id="ref-15"></a>[**※15**](#appendix-15)

# 特定のファイルだけを指定してリントする場合
bundle exec rubocop app/models/user.rb -a
```
...
>
> ⚠️ **注意**: 引数にファイルパスを指定しない場合、プロジェクト内の**全てのファイル**がスキャンの対象となります。意図しない場所が書き換わるのを避けたい場合は、特定のディレクトリやファイルのみを指定して実行しましょう。

### 7.7 不要なテストファイルの整理

ADR 001 に基づき、パスワードリセット機能（`passwords_controller`）は無効化しました。そのため、自動生成された関連テストも不要です。これらを削除してプロジェクトをクリーンに保ちます。
...
### 7.8 セキュリティ・チェック（bundler-audit）

GitHub に送る前に、使用しているライブラリに重大な弱点（脆弱性）が隠れていないかを自分で点検します。プロの開発現場では、CI に指摘される前に手元でこれを確認するのが鉄則です。

[environment/nexus_mfg (feat/chapter02-auth-i18n)]
```bash
# 最新の脆弱性情報を取得（--update）した上で、プロジェクトをスキャンする
bundle exec bundler-audit check --update
```

> 💡 **なぜ `--update` をつけるのか？**
> 新しい脆弱性は日々発見されています。単に `check` するだけでは、手元の古いデータベースにある情報しか検知できません。プッシュ前などの重要なタイミングでは、必ず `--update` をつけて最新の「指名手配リスト」を手に入れるのがプロの習慣です。
>
> 🚀 **プロのバランス感覚**：
> 毎回更新すると実行に時間がかかるため、実務では「一日の作業開始時に一度 `--update` で最新にし、その後の開発中は `check` のみで高速に回す」といった使い分けも行われます。

> 💡 **もしエラーが出たら？**
> `Vulnerabilities found!` と表示された場合は、ライブラリのバージョンが古く、危険な状態です。慌てず、エラーログに従って `bundle update` 等で対応しましょう。詳細は [トラブルシューティング集：Gem の脆弱性解消](../troubleshooting/security/gem_vulnerability.md) を参照してください。

### ✅ 次へ進むための確認事項
- [ ] `bin/rails test:all` でエラーが出ず、GREEN になること。
- [ ] `test/system/.keep` が作成され、Git に追加されていること。
- [ ] 不要なテストファイル（password関連）が削除されていること。
- [ ] `rubocop` で規約違反が残っていないこと。
- [ ] `bundler-audit` でセキュリティ脆弱性が検出されないこと。

---

## <a id="section-8-finish"></a>8. 作業の完了：GitHub への送信と同期

実装が完了しました。最後に、現場で行われる「品質の最終確認」と「歴史の登録（コミット）」を行います。

### 8.1 最終チェック（品質のセルフ監査）

ここまで手順通りに進めていれば、既に全てのチェックを通過しているはずです。しかし、開発現場では、**コミットして歴史に刻む直前に、改めてこれらの点検を自律的に行う習慣**があります。

今後、自分で機能を追加する際も必ず実行すべき **「品質の三関門」** として、改めてその内容を確認しましょう。これらが全てクリアされていれば、あなたのコードは自信を持って世界へ公開できる品質です。

#### 🛡️ 品質の三関門（習慣にすること）
- [ ] **テスト (Minitest)**: `bin/rails test:all` を実行し、全て **GREEN** になっている。
- [ ] **リンター (RuboCop)**: `bundle exec rubocop` または `rubocop` コマンドの実行結果が **`no offenses detected`** になっている。
- [ ] **セキュリティ (audit)**: `bundle exec bundler-audit check --update` で脆弱性が **`Vulnerabilities found!` が表示されない**。

#### 📦 第2章の固有確認
- [ ] **構成 (Git)**: `test/system/.keep` が追加され、不要なテスト（password関連）が削除されている。

---

### 8.2 変更の登録（コミット）と送信（プッシュ）

全てのチェックが完了したら、作業内容を保存し、リモートリポジトリへ同期します。
...
マージの際は、通常のマージで問題ありません。ただし、開発中に試行錯誤してコミットを何度も重ねた場合は、それらを 1 つにまとめて歴史を綺麗にする「Squash merge」が推奨されます <a id="ref-13"></a>[**※13**](#appendix-13)。
...
### <a id="appendix-11"></a>**[※11 辞書ファイル（ja.yml）の肥大化対策：フォルダ分割戦略](#ref-11)**

開発が進み、一つの `ja.yml` が数千行を超えると保守が困難になります。その際は、以下のように機能単位でファイルを分割するのがプロの定石です。

#### 1. 推奨されるフォルダ構成
`config/locales/` の下に言語ごとのフォルダ（`ja/`）を作り、役割ごとに分割します。

```text
config/locales/
├── ja/
│   ├── models.yml    # モデル・属性の翻訳
│   ├── views.yml     # 画面固有の文言
│   ├── enums.yml     # Enum値の翻訳
│   └── common.yml    # ボタン名などの共通文言
└── ja.yml            # Rails標準の共通設定など
```

#### 2. ファイル記述のルール
分割しても、**全てのファイルの最上位階層は `ja:`（言語コード）から始める**必要があります。

```yaml
# config/locales/ja/models.yml の例
ja:
  activerecord:
    models:
      user: "ユーザー"
...
#### 3. フォルダを認識させるための必須設定
Rails はデフォルトでは `config/locales/` 直下のファイルしか読み込みません。フォルダを掘った場合は、`config/application.rb` に以下の設定を追記する必要があります。
...
module NexusMfg
  class Application < Rails::Application
    # ...略...
    # locales 以下の全サブフォルダにある yml ファイルを読み込む設定
...
#### 5. 実行範囲のコントロール：全体 vs 局所
プロの現場では、状況に応じてリントの範囲を使い分けます。
- **プロジェクト全体実行**: プルリクエストの作成前や CI 環境で行います。プロジェクト全体の品質が一定に保たれているかを最終確認する「総点検」の役割です。
- **ファイル/ディレクトリ指定実行**: 自分が今編集しているファイルに対してのみ行います。余計なファイルが書き換わるリスクを避け、リントの待ち時間を最小限に抑えることで、開発のテンポを維持できます。

👉 [Resource Hub (RuboCop)](../resource_hub.md#tech-rubocop)
### <a id="appendix-16"></a>**[※16 CI 環境の役割とテストの使い分け：開発のリズムを守るために](#ref-16)**

#### 1. なぜ手元の `test` では不十分なのか？
Rails のテストは、実行速度と信頼性のバランスをとるために以下のように設計されています。

- **ユニット/統合テスト (`bin/rails test`)**:
  データベースとロジックを直接チェックします。ブラウザを通さないため**非常に高速**で、開発者が「一歩進むたびに叩く」のに適しています。
- **システムテスト (`bin/rails test:system`)**:
  実際にブラウザ（Chrome等）を起動して JS の動きなどをチェックします。**非常に重く時間がかかる**ため、デフォルトのテストからは除外されています。
- **全テスト (`bin/rails test:all`)**:
  システムテストを含む全ての不備をあぶり出します。**「品質の最終関門」**として、プッシュ前には必ず実行すべきコマンドです。

#### 2. CI（継続的インテグレーション）と「空フォルダの罠」
GitHub Actions などの CI 環境は、コードを受け取ると「真っさらな状態」からテストを始めます。ここで初心者が最もハマるのが **Git の空フォルダ無視** です。

- **現象**: 手元の Mac には `test/system` があるのに、GitHub 上にはフォルダがないため `LoadError` になる。
- **Rails のお作法**: Rails はテストを開始する際、「ここにはシステムテストがあるはずだ」という場所をまず **読み込み（Load）** に行きます。このとき、物理的なフォルダが存在しないと「あるはずの場所がない！」と判断して、中身を確認する前にエラーで止まってしまいます。
- **対策**: `.keep` ファイルを置いて Git に「ここは空じゃない、意味がある構造だ」と教えることで、CI 環境でも正しくテストが実行できるようになります。**「箱（フォルダ）」さえあれば、中身が空であっても Rails は「テストなし」として正常にスキップしてくれます。**
    - 💡 **`.keep` とは？**: 実は Git の機能ではなく、エンジニアが作った「慣習」です。Git はファイルがないフォルダを無視するため、中身が空のファイル（名前は何でも良いのですが、意図が伝わりやすい `.keep` が一般的）を置くことで、フォルダ構造を無理やり Git に認識させています。
