...
まずは、ジェネレーターが吐き出した「Railsの標準（英語）」のUIと、開発者が直接いじった「日本語のハードコード」が混在する状態で、機能実装を行います。
その後に、**「なぜ体系的な日本語化（I18n）が必要なのか」**を実機で体験しながら、現場で使える強固な基盤を整えます。その過程で、一貫性のない画面や管理上のコストという**「不利益（Pain）」**をあえて体験し、I18n 管理の必然性を理解していただきます。
...
7. [作業の完了：GitHub への送信と同期](#section-7-finish)
8. [学習ノート](#section-8-※)
...
まず、認証の核となるパスワードのハッシュ化ライブラリ **bcrypt** <a id="ref-2"></a>[**※2**](#appendix-2) を有効化します。
...
### ✅ 次へ進むための確認事項
- [ ] データベースのマイグレーションとシードがエラーなく完了していること。
...
### ✅ 次へ進むための確認事項
- [ ] ブラウザでログイン画面が表示され、英語と日本語が混在していることを目視で確認できたこと。
...
## <a id="section-7-finish"></a>7. 作業の完了：GitHub への送信と同期
